...
> Deduplication rule: each term appears once globally. When a term appears in multiple section dictionaries, it is kept in its first/primary topic location and the additional topic locations are listed in `Also appears in`, without repeating the explanation.
...
| Term / Acronym / Keyword | Meaning | Also appears in |
|---|---|---|
| 3-tier architecture | Traditional infrastructure model separating compute, network, and storage into independent layers. | — |
| AHV | Acropolis Hypervisor; Nutanix native virtualization hypervisor integrated with AOS and Prism. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| AOS | Acropolis Operating System; Nutanix software layer providing distributed storage, data services, resiliency, upgrades, and cluster intelligence. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| Availability domain | Failure boundary such as disk, node, block, rack, or site used to reason about resiliency. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| Block | Physical Nutanix chassis/enclosure that may contain one or more nodes and can be treated as a failure domain. | AHV and Virtualization / VM Lifecycle, Migration, HA, and Placement; AOS and Storage / Core AOS Storage Architecture |
| Block awareness | Replica placement strategy designed to reduce impact from a block-level failure. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| Cluster | Group of Nutanix nodes operating together as one distributed infrastructure platform. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
...
| CVM | Controller Virtual Machine; critical Nutanix VM on each node providing storage and cluster services. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| Data locality | Nutanix behavior that serves VM I/O near the VM location when possible to reduce latency and network traffic. | AOS and Storage / Data Path, Locality, and Performance |
| Distributed system | System composed of multiple cooperating components; central mental model for Nutanix clusters. | — |
| DSF | Distributed Storage Fabric; Nutanix distributed storage layer aggregating local disks into resilient shared storage. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
...
| HCI | Hyperconverged Infrastructure; architecture combining compute, storage, virtualization, networking integration, and management in a software-defined cluster. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| Hybrid node | Node with mixed flash and disk storage, typically SSD/NVMe plus HDD. | — |
| Node | Physical server in a Nutanix cluster contributing compute and usually storage. | AOS and Storage / Core AOS Storage Architecture |
| Prism | Nutanix management and monitoring interface for health, alerts, capacity, performance, tasks, and operations. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| Prism Central | Centralized Nutanix management plane for multiple clusters and advanced operational capabilities. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| Prism Element | Cluster-level Nutanix management interface for a single cluster. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
...
| Backup | Copy of data or workload state kept for recovery purposes. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
...
| Capacity headroom | Free resource margin reserved for growth, rebuild, snapshots, recovery, and safe operations. | AOS and Storage / Resiliency, Replication, and Failure Domains |
...
| Compression | Storage efficiency technique that reduces the physical size of stored data. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
...
| Data placement | Process of deciding where data and replicas are stored across the cluster. | AOS and Storage / Data Path, Locality, and Performance |
| Data protection | Mechanisms such as snapshots, replication, backups, and redundancy used to prevent data loss. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| Data resiliency | Ability to preserve availability and data integrity after failures. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| Deduplication | Storage efficiency technique that avoids storing duplicate data blocks. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
...
| EC-X | Erasure Coding Extension; Nutanix capacity optimization feature that reduces overhead compared with full replication. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
| Erasure coding | Data protection and capacity-efficiency technique using fragments and parity rather than full extra copies. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
| Failover | Moving or restarting workloads/services on a backup component, host, cluster, or site after failure. | AHV and Virtualization / VM Lifecycle, Migration, HA, and Placement |
| Fault tolerance | Ability to continue operating after component failure. | AHV and Virtualization / VMware, Hyper-V, and Migration Vocabulary; AOS and Storage / Resiliency, Replication, and Failure Domains |
| HDD | Hard Disk Drive; magnetic disk storage often used for capacity tiers in hybrid systems. | AOS and Storage / Data Path, Locality, and Performance |
| Hotspot | Localized resource concentration causing performance degradation. | — |
| I/O | Input/Output; read and write operations between workloads and storage or network systems. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Data Path, Locality, and Performance |
| IOPS | Input/Output Operations Per Second; metric for storage read/write operation rate. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Data Path, Locality, and Performance |
| NAS | Network-Attached Storage; file-level storage accessed over a network. | — |
| NFS | Network File System; file protocol often used for VMware datastores. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Protocols, Hypervisors, and Integrations |
| NVMe | High-performance SSD interface/protocol used for low-latency storage. | AOS and Storage / Data Path, Locality, and Performance |
| Object storage | Storage model where data is stored as objects with metadata. | — |
| Physical disk | Actual storage hardware such as HDD, SSD, or NVMe. | AOS and Storage / Data Path, Locality, and Performance |
| QoS | Quality of Service; resource control or prioritization mechanism. | AOS and Storage / Data Path, Locality, and Performance |
| RAID | Traditional disk redundancy model; useful contrast with Nutanix distributed resiliency. | — |
| Raw capacity | Total physical disk capacity before resiliency, metadata, and operational overheads. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
| Rebalance / Rebalancing | Redistribution of data or load across nodes or disks. | — |
| Rebuild | Reconstruction of data redundancy after disk or node failure. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| Replication | Maintaining additional copies of data for resilience or disaster recovery. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| Replication Factor / RF | Number of data copies maintained for protection. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| RF2 | Replication Factor 2; two copies of data. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| RF3 | Replication Factor 3; three copies of data for higher resiliency. | AOS and Storage / Resiliency, Replication, and Failure Domains |
| SAN | Storage Area Network; dedicated storage architecture providing block-level storage. | AOS and Storage / Protocols, Hypervisors, and Integrations |
| Snapshot | Point-in-time copy of data or VM state. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| SSD | Solid-State Drive; flash-based storage device. | AOS and Storage / Data Path, Locality, and Performance |
| Storage array | Dedicated external storage system used in traditional architectures. | — |
| Storage controller | Component that manages storage I/O and data services. | AOS and Storage / Core AOS Storage Architecture |
| Storage fabric | Distributed storage layer coordinating storage resources. | — |
| Storage pool | Logical or physical aggregation of storage resources across a cluster. | AOS and Storage / Core AOS Storage Architecture |
| Thin provisioning | Allocating logical capacity without reserving all physical capacity upfront. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
| Tiering | Placement or movement of data across media types such as NVMe, SSD, and HDD. | AOS and Storage / Data Path, Locality, and Performance |
| Usable capacity | Capacity available after metadata, resiliency, rebuild reserve, and other overheads. | AOS and Storage / Capacity, Data Efficiency, and Storage Economics |
| VDI | Virtual Desktop Infrastructure. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| vDisk | Virtual disk object consumed by a VM. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Core AOS Storage Architecture |
...
| Term / Acronym / Keyword | Meaning | Also appears in |
|---|---|---|
| ADS | Acropolis Dynamic Scheduler; Nutanix service for workload placement and resource usage optimization. | AHV and Virtualization / Core AHV and Virtualization Architecture |
...
| CPU | Central Processing Unit; physical compute resource. | AHV and Virtualization / CPU, Memory, NUMA, and Resource Scheduling |
| CPU contention | Multiple workloads competing for limited physical CPU resources. | AHV and Virtualization / CPU, Memory, NUMA, and Resource Scheduling |
| CPU Ready | VMware metric showing time a VM is ready to run but waiting for physical CPU resources. | AHV and Virtualization / CPU, Memory, NUMA, and Resource Scheduling |
| ESXi | VMware enterprise bare-metal hypervisor; Nutanix can run with AHV or ESXi in supported deployments. | AHV and Virtualization / VMware, Hyper-V, and Migration Vocabulary; AOS and Storage / Protocols, Hypervisors, and Integrations |
| HA | High Availability; design principle keeping services running or restarting workloads after failures. | AHV and Virtualization / VM Lifecycle, Migration, HA, and Placement |
| Host | Physical server running a hypervisor and hosting VMs. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| Host isolation | Condition where a host loses management or cluster connectivity and may trigger HA behavior. | — |
| Hyper-V | Microsoft hypervisor for running virtual machines. | AHV and Virtualization / Core AHV and Virtualization Architecture; AHV and Virtualization / VMware, Hyper-V, and Migration Vocabulary; AOS and Storage / Protocols, Hypervisors, and Integrations |
| Hypervisor | Software layer that runs and manages virtual machines. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Core AOS Storage Architecture |
| KVM | Kernel-based Virtual Machine; Linux virtualization technology underlying AHV concepts. | AHV and Virtualization / Core AHV and Virtualization Architecture |
| Live migration | Moving a running VM from one host to another with minimal or no downtime. | AHV and Virtualization / VM Lifecycle, Migration, HA, and Placement |
...
| NCI | Nutanix Cloud Infrastructure; Nutanix platform including AOS, AHV, DR, and related capabilities. | AHV and Virtualization / Core AHV and Virtualization Architecture |
| Noisy neighbor | Workload consuming excessive shared resources and affecting others. | — |
| NUMA | Non-Uniform Memory Access; server memory architecture relevant to large VM sizing and performance. | AHV and Virtualization / CPU, Memory, NUMA, and Resource Scheduling |
| PDL | Permanent Device Loss; VMware condition where a storage device is permanently unavailable. | AHV and Virtualization / VMware, Hyper-V, and Migration Vocabulary |
| VM | Virtual Machine; software-defined server running on a hypervisor. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| VMware | Enterprise virtualization and cloud infrastructure platform/vendor. | AHV and Virtualization / Core AHV and Virtualization Architecture; AOS and Storage / Protocols, Hypervisors, and Integrations |
...
| DNS | Domain Name System; service resolving names to IP addresses. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |
| East-west traffic | Traffic between systems inside a datacenter, cluster, or application environment. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |
...
| Firewall | Security device or rule set controlling network traffic. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |
| HBA | Host Bus Adapter; adapter connecting servers to storage networks, commonly Fibre Channel. | — |
| IP | Internet Protocol; core addressing and routing protocol for network communication. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |
| IPMI | Intelligent Platform Management Interface; out-of-band server management interface. | — |
| iSCSI | Internet Small Computer Systems Interface; protocol carrying SCSI commands over IP networks. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Protocols, Hypervisors, and Integrations |
| Jumbo frames | Ethernet frames larger than standard MTU, often used in storage or high-throughput networks. | — |
| LACP | Link Aggregation Control Protocol; bundles multiple physical network links into one logical link. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration; AOS and Storage / Protocols, Hypervisors, and Integrations |
...
| Management plane | Control, configuration, monitoring, and administration layer. | AHV and Virtualization / Core AHV and Virtualization Architecture |
| Migration traffic | Traffic generated during VM live migration or storage migration. | — |
| MTU | Maximum Transmission Unit; maximum packet size on a network path. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |
| Multipathing | Use of multiple storage paths for redundancy and load balancing. | — |
| NIC | Network Interface Card; adapter connecting a server to a network. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |
| North-south traffic | Traffic entering or leaving a datacenter, cluster, or application environment. | — |
| NTP | Network Time Protocol; clock synchronization protocol. | AHV and Virtualization / VMware, Hyper-V, and Migration Vocabulary |
...
| VLAN | Virtual Local Area Network; Layer 2 segmentation method. | AHV and Virtualization / Networking, Storage Devices, and Guest Integration |

### Operations, Support, and Incident Management

| Term / Acronym / Keyword | Meaning | Also appears in |
|---|---|---|
| Alert | Notification indicating a health, capacity, performance, or resiliency condition. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Availability | Ability of a system or service to remain operational and accessible. | AHV and Virtualization / VM Lifecycle, Migration, HA, and Placement; AOS and Storage / Resiliency, Replication, and Failure Domains |
| Blast radius | Scope of impact of an incident across users, services, VMs, hosts, clusters, or sites. | AHV and Virtualization / VM Lifecycle, Migration, HA, and Placement |
...
| Business impact | Practical effect of an issue on customer operations, users, revenue, or SLA. | AOS and Storage / Operations, Support, and Escalation |
...
| Confluence | Documentation and knowledge-management platform. | AHV and Virtualization / Operations, Support, and Escalation |
...
| Enterprise support | Support for business-critical customer environments with strong SLAs and escalation expectations. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Escalation | Raising an issue to higher support, SRE, engineering, management, TAM, or account team. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Field Engineer | Engineer responsible for on-site hardware replacement or repair. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Firmware | Low-level software embedded in hardware devices such as NICs, BIOS, disks, or controllers. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| GCP | Google Cloud Platform; public cloud provider relevant to hybrid cloud discussions. | AOS and Storage / Protocols, Hypervisors, and Integrations |
| Grafana | Monitoring and dashboarding platform for metrics and alerts. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Protocols, Hypervisors, and Integrations |
...
| Incident management | Process for detecting, triaging, resolving, communicating, and reviewing incidents. | AHV and Virtualization / Operations, Support, and Escalation |
...
| Jira | Issue and workflow tracking system used for incidents, escalations, bugs, and tasks. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| K8s / Kubernetes | Container orchestration platform relevant to distributed systems and workload operations. | AOS and Storage / Protocols, Hypervisors, and Integrations |
| Kibana | Log analysis and visualization tool. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Protocols, Hypervisors, and Integrations |
| KPI | Key Performance Indicator; metric used to track system, service, or team performance. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Latency | Time taken for an operation to complete. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Data Path, Locality, and Performance |
| LCM | Life Cycle Manager; Nutanix software/firmware lifecycle and update management. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Protocols, Hypervisors, and Integrations |
| Linux | Operating system widely used in infrastructure and cloud environments. | AHV and Virtualization / Core AHV and Virtualization Architecture |
| Monitoring | Collection and analysis of metrics, logs, events, and alerts. | AOS and Storage / Operations, Support, and Escalation |
| MTTR | Mean Time To Resolution/Restore/Repair; time required to recover service or resolve an incident. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| NCC | Nutanix Cluster Check; health-check utility for Nutanix clusters. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Observability | Ability to understand system state through metrics, logs, traces, alerts, and events. | AOS and Storage / Operations, Support, and Escalation |
| P1 | Priority 1 incident; usually critical production impact. | AHV and Virtualization / Operations, Support, and Escalation |
| RCA | Root Cause Analysis; formal explanation of what happened, why, impact, and prevention. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| RFO | Reason for Outage; explanation of outage cause. | AOS and Storage / Operations, Support, and Escalation |
| Runbook | Documented operational procedure for troubleshooting or recovery. | AOS and Storage / Operations, Support, and Escalation |
| SaaS | Software as a Service. | — |
| Sev1 | Severity 1; critical incident with major production or business impact. | AHV and Virtualization / Operations, Support, and Escalation |
| SLA | Service Level Agreement; contractual or operational service commitment. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| SRE | Site Reliability Engineering/Engineer; reliability, automation, performance, and operations role/discipline. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Support bundle | Diagnostic package collected for troubleshooting. | AOS and Storage / Operations, Support, and Escalation |
| TAM | Technical Account Manager; customer-facing technical relationship owner. | AOS and Storage / Operations, Support, and Escalation |
| Throughput | Amount of data transferred over time, often MB/s or GB/s. | AOS and Storage / Data Path, Locality, and Performance |
| Triage | Structured initial assessment of impact, scope, urgency, symptoms, and next action. | AHV and Virtualization / Operations, Support, and Escalation; AOS and Storage / Operations, Support, and Escalation |
| Worldwide Support | Global support organization operating across regions and time zones. | AOS and Storage / Operations, Support, and Escalation |
...
| Controller VM | Nutanix VM running storage and cluster services on each node; also called CVM. | AOS and Storage / Core AOS Storage Architecture |
| Datastore | Logical storage location for VM files, especially in VMware contexts. | AOS and Storage / Protocols, Hypervisors, and Integrations |
| Distributed Storage Fabric | Nutanix distributed storage layer across cluster nodes. | AOS and Storage / Core AOS Storage Architecture |
| Guest OS | Operating system running inside a VM. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| libvirt | API/daemon commonly used to manage KVM/QEMU VMs. | — |
| nCLI | Nutanix command-line interface for cluster and platform management. | AOS and Storage / Protocols, Hypervisors, and Integrations |
...
| Maintenance Mode | Operational state used to prepare a host for planned work by evacuating workloads. | AOS and Storage / Operations, Support, and Escalation |
...
| PCI Passthrough | Direct PCI device assignment to a VM; can prevent migration. | AOS and Storage / Protocols, Hypervisors, and Integrations |
...
| Replica | Redundant instance of a service or application component. | AOS and Storage / Resiliency, Replication, and Failure Domains |
...
| NGT | Nutanix Guest Tools; guest integration software for Nutanix VMs. | AHV and Virtualization / VMware, Hyper-V, and Migration Vocabulary |
...
| Protection Domain | Nutanix data protection construct used for snapshot/replication policies. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| Quiescing | Pausing or coordinating writes so snapshots are more consistent. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| RDMA | Remote Direct Memory Access; can reduce CPU/network overhead in some high-performance scenarios. | AOS and Storage / Data Path, Locality, and Performance |
| RPO | Recovery Point Objective; acceptable data loss measured in time. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| RTO | Recovery Time Objective; target time to restore service. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| Snapshot/replication impact | Performance or behavior effect from snapshots or replication during time-correlated cases. | — |
| SSR | Self-Service Restore; Nutanix feature for file-level recovery from snapshots/recovery points. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
| Storage Capacity | Available storage required on a destination or cluster for VM operations. | — |
| Storage latency | Delay in storage read/write operations. | AOS and Storage / Data Path, Locality, and Performance |
...
| VSS | Volume Shadow Copy Service; Windows framework for consistent snapshots. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
...
| Disaster Recovery / DR | Recovery strategy for site, platform, or major infrastructure failure. | AOS and Storage / Snapshots, Clones, Backup, and Recovery |
...
| vSphere | VMware virtualization suite including ESXi and vCenter. | AOS and Storage / Protocols, Hypervisors, and Integrations |
...
| Foreground I/O | Active application read/write traffic. | AOS and Storage / Data Path, Locality, and Performance |
